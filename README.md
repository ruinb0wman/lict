...
- [ ] 实现 `data:getPath` - 获取用户数据目录
- [ ] 实现 `favorites:load` / `favorites:save`
- [ ] 实现 `history:load` / `history:save`
- [ ] 在 `electron/preload.ts` 暴露 IPC API

#### 2.2 收藏功能

- [ ] 创建 `src/stores/favoritesStore.ts`
- [ ] 实现「收藏」按钮点击事件
- [ ] 实现添加/取消收藏逻辑
- [ ] 收藏按钮状态根据收藏状态变化
...
- [ ] 创建收藏页面 `src/pages/Favorites.tsx`
- [ ] 实现收藏列表展示（单词、主要翻译、时间）
- [ ] 实现搜索收藏单词功能
- [ ] 实现取消收藏按钮
- [ ] 点击单词跳转到查询结果页
...
- [ ] 修改 API 调用逻辑，自动识别单词/句子
- [ ] 句子查询只返回翻译，不返回例句
- [ ] 调整 UI 展示：句子翻译样式

#### 3.2 自动读取剪切板

- [ ] 在 `electron/main.ts` 监听窗口显示事件
- [ ] 窗口显示时读取剪切板内容
- [ ] 判断是否为英文内容（正则匹配）
- [ ] 发送剪切板内容到渲染进程
- [ ] 渲染进程自动填充并触发查询
...
- [ ] 创建复习页面 `src/pages/Review.tsx`
- [ ] 实现卡片翻转效果（CSS 3D transform）
- [ ] 正面显示单词，背面显示翻译和例句
- [ ] 实现「认识」「不认识」按钮
- [ ] 记录复习次数和上次复习时间
- [ ] 随机抽取收藏的单词
...
- [ ] 在 `electron/main.ts` 注册全局快捷键 `Alt+D`
- [ ] 快捷键触发时显示/隐藏窗口
- [ ] 将快捷键配置化（可自定义）
- [ ] 添加「Esc」隐藏窗口功能
...
- [ ] 创建底部导航栏组件 `src/components/BottomNav.tsx`
- [ ] 实现页面切换动画
- [ ] 当前页面高亮显示

#### 4.2 样式优化

- [ ] 添加 Tailwind CSS 或自定义 CSS 变量
- [ ] 实现主色调（蓝色 #3b82f6）
- [ ] 添加 Loading 动画
- [ ] 添加 Toast 提示组件
- [ ] 优化滚动条样式
