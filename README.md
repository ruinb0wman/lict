...
| 💾 本地缓存 | 使用 LowDB 存储查询过的单词，重复查询时直接读取本地数据 |
...
- 使用 LowDB 存储所有查询过的单词数据
- 重复查询时优先从本地数据库读取，不访问 API
- 首次查询时从 API 获取并保存到 LowDB
...
├── favorites.json      # 收藏单词
├── history.json        # 查询历史
└── config.json         # 用户配置（electron-store 管理）
```

### 存储实现

- 收藏和历史使用 JSON 文件读写
- 主进程提供 IPC API 供渲染进程调用
- 配置使用 `electron-store` 库
...
- [x] 修改 `electron/main.ts`：添加 IPC 处理器
- [x] 实现 `data:getPath` - 获取用户数据目录
- [x] 实现 `favorites:load` / `favorites:save`
- [x] 实现 `history:load` / `history:save`
- [x] 在 `electron/preload.ts` 暴露 IPC API
